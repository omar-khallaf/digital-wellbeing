...
[overview event pipeline](./README.md#event-processing--true-event-log).
...
| --------------------------------- | ------------------------------------------ | ------------------------------------------------------------- |
| ListPolicies(filter)              | Returns policies for filter uid (all if 0) | Returns policies where user_id == caller_uid. filter ignored. |
| CreatePolicy(input)               | Any user_id                                | Only if input.user_id == caller_uid                           |
...
| SetAppCategory(app_class, cat_id) | Applies                                    | Applies                                                       |
...
| Scope           | Mode    | user_id / created_by       | Authorization                                               |
| --------------- | ------- | -------------------------- | ----------------------------------------------------------- |
| MultiUser       | System  | Any uid (root sets freely) | Full root-vs-user matrix (table above)                      |
| SingleUser(uid) | Session | Always uid (the daemon's)  | Pass-through: caller is always uid -> allow; filter ignored |
...
Root creates a policy for user 1000: user_id = 1000, created_by = 0

User 1000 sees this policy in ListPolicies (user_id matches) but CANNOT update
or delete it (created_by != own uid) -> user sees the limit applied to them but
cannot remove it

User 1000 creates their own policy: user_id = 1000, created_by = 1000
...
The EnforcerActor runs one evaluation cycle per focus event. It:

1. Gets the uid from the plugin's D-Bus connection credentials.
2. Queries policies WHERE user_id = uid (schedule filtering done in SQL).
3. Queries `daily_usage_by_app` and `daily_usage_by_category` WHERE user_id =
   uid.
4. Evaluates policies → applies block / notify / unrestricted.
...
each event. Stored generated columns `timestamp` and `app_class` are part of the
...
The `daily_usage_by_app` table includes a `user_id` column. The primary key is
`(date, user_id, app_id)` to support per-user scoping. `daily_usage_by_category`
follows the same pattern with `(date, user_id, category_id)`.

### policies table

The `policies` table includes `user_id` and `created_by` columns. `user_id` is
the uid the policy applies to (RBAC scoping). `created_by` is the uid that
created the policy (RBAC ownership). An index on `user_id` supports the per-user
policy query.
