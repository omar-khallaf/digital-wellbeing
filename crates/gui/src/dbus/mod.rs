...
#[proxy(
    interface = "org.wellbeing.v1.Daemon",
    default_service = "org.wellbeing.v1.Daemon",
    default_path = "/org/wellbeing/Daemon"
)]
trait Daemon {
    /// ═══ Policy CRUD ═══
...
pub struct DaemonClient {
    ...
    conn: Connection,
    usage_cache: Arc<ClientCache<String, Vec<DailyUsageEntry>>>,
...
impl DaemonClient {
    ...
    pub async fn connect() -> Result<Self> {
        ...
        Ok(Self {
            proxy,
            conn,
            usage_cache: Arc::new(ClientCache::new(Duration::from_millis(500))),
    ...
    pub async fn get_daily_usage(&self, date: &str, user_id: u32) -> Result<Vec<DailyUsageEntry>> {
        let key = format!("usage:{}:{}", date, user_id);
        if let Some(cached) = self.usage_cache.get(&key) {
            return Ok(cached);
        }
        let entries = self.proxy.get_daily_usage(date, user_id).await?;
        self.usage_cache.set(key, entries.clone());
        Ok(entries)
    }
...
/// Resolve which bus hosts `org.wellbeing.v1.Daemon`.
async fn resolve_daemon_bus() -> Option<Connection> {
    ...
    if let Some(ref c) = sys
        && start_service_by_name(c, "org.wellbeing.v1.Daemon").await
        && name_owner_present(c, "org.wellbeing.v1.Daemon").await
    {
