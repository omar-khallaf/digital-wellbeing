...
auto nameHasOwner(sdbus::IConnection &conn, const std::string &name) -> bool {
    try {
        auto proxy = sdbus::createProxy(conn, sdbus::ServiceName{DBUS_INTERFACE}, sdbus::ObjectPath{DBUS_OBJECT_PATH});
        bool owned = false;
        proxy->callMethod(NAME_HAS_OWNER_METHOD).onInterface(DBUS_INTERFACE).withArguments(name).storeResultsTo(owned);
        return owned;
